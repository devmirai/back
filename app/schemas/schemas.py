...
class EventCreateSchema(Schema):
    ...
    description = fields.Str(required=True)  # <-- Cambia a requerido
...
class EventUpdateSchema(Schema):
    ...
    isActive = fields.Bool(allow_none=True)  # <-- Añade esta línea
...
class PaymentMethodSchema(Schema):
    ...
    type = fields.Str(required=True, validate=validate.OneOf(['CREDIT_CARD']))  # <-- Solo acepta CREDIT_CARD
    ...
    cardNumber = fields.Str(allow_none=True, validate=validate.Length(equal=16))
